...
const char modeChar[] = {'k', 'e', 'd', 'b', 'a', 'm', 't'};
const char *modeForm[] = {
	"<key size> <KeyFileName>",
	"<m> <KeyFileName>",
	"<C_m> <KeyFileName>",
	"<m> <KeyFileName>",
...
const int minArgs[] = {4, 4, 4, 4, 5, 5, 5};
const int maxArgs[] = {4, 4, 4, 4, 7, 7, 7};
...
void modeHomomorphicEquality(int argc, char *argv[]);
...
int parseCiphertextOrConvertFromPlaintext(BIGD ciphertext, int argc, char *argv[], int pos, FHEv1_EncryptionKey *ek);

void putKey(FHEv1_Key *key, char *filepath);
FHEv1_Key *getKey(char *filepath);
...
int main(int argc, char *argv[]) {
	...
	switch(m) {
		...
		case M_M_HOMOMULT:
			...
			break;
		case M_T_HOMOEQ:
			modeHomomorphicEquality(argc, argv);
			break;
		default:
...
void modeKeygen(int argc, char *argv[]) {
	...
	FHEv1_Key *key = FHEv1_Key_createFromKeyLen(numDigits);
	putKey(key, argv[3]);
	FHEv1_Key_destroy(key);
}
...
void modeEncrypt(int argc, char *argv[]) {
	...
	FHEv1_Key *key = getKey(argv[3]);

	FHEv1_encrypt(ciphertext, plaintext, &(key->encryptionKey));
...
void modeDecrypt(int argc, char *argv[]) {
	...
	FHEv1_decrypt(plaintext, ciphertext, &(key->encryptionKey));
...
void modeHomomorphicAdd(int argc, char *argv[]) {
	...
	BIGD ct2 = bdNew();
	BIGD ctr = bdNew();
	FHEv1_Key *key = getKey(argv[argc - 1]);
	int pos = 2;
	pos = parseCiphertextOrConvertFromPlaintext(ct1, argc, argv, pos, &(key->encryptionKey));
	parseCiphertextOrConvertFromPlaintext(ct2, argc, argv, pos, &(key->encryptionKey));

	FHEv1_homomorphicAdd(ctr, ct1, ct2, &(key->operationalKey));
...
void modeHomomorphicMultiply(int argc, char *argv[]) {
	...
	FHEv1_homomorphicMultiply(ctr, ct1, ct2, &(key->operationalKey));
...
void modeHomomorphicEquality(int argc, char *argv[]) {
	...
	if(FHEv1_homomorphicEquality(ct1, ct2, &(key->operationalKey))) {
		printf("true\n");
	} else {
		printf("false\n");
	}
...
int parseCiphertextOrConvertFromPlaintext(BIGD ciphertext, int argc, char *argv[], int pos, FHEv1_EncryptionKey *ek) {
	...
	if(strlen(argv[pos]) == 2 && argv[pos][0] == '-' && argv[pos][1] == 'e') {
		...
		FHEv1_encrypt(ciphertext, plaintext, ek);
		bdFree(&plaintext);
		result = pos + 2;
	} else {
...
void putKey(FHEv1_Key *key, char *filepath) {
	...
	char *buf = FHEv1_Key_toNewString(key);
...
FHEv1_Key *getKey(char *filepath) {
	...
	FHEv1_Key *result = FHEv1_Key_createFromString(buf);
